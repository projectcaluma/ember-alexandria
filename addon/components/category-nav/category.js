...
export default class CategoryNavCategoryComponent extends Component {
  ...
  async copyFailedMoves(documentInfos, newCategory) {
    const moveConfirmed = await confirm(
      this.intl.t(
        documentInfos.length === 1
          ? "alexandria.errors.move-failed"
          : "alexandria.errors.move-failed-multiple",
        {
    ...
    if (moveConfirmed) {
      ...
      const copyCountSuccess = copyStates.filter((s) => s !== false).length;
      const copyCountFailed = copyStates.filter((s) => s === false).length;
