  ...
  hooks.before(function () {
    ...
    this.assertRequests = function (method, path, assertFuncs) {
      ...
      pretender[method.toLowerCase()](path, function (request) {
        assertFuncs.shift()(request);

        if (!assertFuncs.length) {
          // Reset the get handler for this path to
          // the default configured in mirage config.
          pretender[method.toLowerCase()](path, originalHandler);
        }
...
function getAndCacheDefaultHandler(pretender, method, path, cache) {
  ...
  try {
    ...
    if (error instanceof TypeError) {
      throw `assertRequest Intercepted: ${method} ${path}:\nYou tried to assert a url which has no handler configured.\nCheck you mirage config if all routes are defined correctly (and dont forget to add the id of the model if needed to the assertion url).`;
    }
    throw error;
  }
